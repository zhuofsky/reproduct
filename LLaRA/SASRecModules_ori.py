...
class MultiHeadAttention_my(nn.Module):
    ...
    def forward(self, queries, keys, attention_mask=None):
        ...
        # 矩阵分割
        Q_ = Q.reshape(batch_size, -1, self.num_heads, self.hidden_size).transpose(1, 2)
        K_ = K.reshape(batch_size, -1, self.num_heads, self.hidden_size).transpose(1, 2)
        V_ = V.reshape(batch_size, -1, self.num_heads, self.hidden_size).transpose(1, 2)

        # scaled dot-product attention
        attention_scores = torch.matmul(Q_, K_.transpose(-2, -1)) / self.hidden_size**0.5
        if attention_mask is not None:
            # 这里的attention_mask只有
            attention_mask = attention_mask.unsqueeze(1).unsqueeze(2).expand(-1, self.num_heads, seq_len, -1)
            attention_scores = attention_scores.masked_fill(attention_mask == 0, -1e9)
        else:
            # 填充掩码
            padding_mask = (queries!=0).int().to(self.device)
            # (batch_size, seq_len)
            padding_mask = padding_mask.unsqueeze(1).unsqueeze(2).expand(-1, self.num_heads, seq_len, -1)
        ...
        attention_output = torch.matmul(attention_prob, V_).transpose(1, 2).reshape(batch_size, -1, self.hidden_size)
